...
action <- data.frame(variable = colnames(df)
                   ,action_group = rep("collapse", ncol(df))
                   ,parameters =  rep("c(0,1,2,3)", ncol(df))
)
```
...
# Test collapsing using action group for reference
```{r tests4}
df1 <- collapseVarAll(df, action=action)
``` 
Expected outcome for each variable:
0  1  1  0  2  3  0  2 NA
