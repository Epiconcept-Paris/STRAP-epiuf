...
dictionary1 <- data.frame(source_name = colnames(df)
                         ,generic_name = colnames(df)
                         ,type = c("character", "numeric")
                         ,dico = rep(NA, ncol(df))
                         ,unknowns = c(NA, 3)
                         ,description = c("a character variable", "a numeric variable")
                         ,comments = rep(NA, ncol(df))
)

dictionary2 <- data.frame(source_name = colnames(df)
                         ,generic_name = colnames(df)
                         ,type = c("character", "numeric")
                         ,dico = rep(NA, ncol(df))
                         ,unknowns = c("", "2,3")
...
```{r tests4}
df1 <- applyNAAll(df,dictionary=dictionary1)
# CharacterVar 7 conversions
# NumericVar 3 conversions

df2 <- applyNAAll(df,dictionary=dictionary2)
# CharacterVar 7 conversions
# NumericVar 7 conversions
``` 
