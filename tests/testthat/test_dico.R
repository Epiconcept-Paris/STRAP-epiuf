...
test_that("Base dico function",{
  file1 <- externalFile("genericdictionary.xlsx")
  openDictionary(file1)
  val <- getAnyDictionaryValue("gen4",search = "generic_name",value="description")
  expect_equal(val,"Binary variable")
  val <- getAnyDictionaryValue("gen12",search = "generic_name",value="description")
  expect_equal(val,NA,label="Missing variable should give NA")
  expect_warning(val <- getAnyDictionaryValue("gen4",search = "badname",value="description"))
