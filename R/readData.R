...
readData <- function(filename = "", factorise = FALSE, lowercase= FALSE, label = NULL,...) {
  ...
  if (file.exists(filename)) {
    ...
    if (ext == "csv") {
      ...
      _testline <- readLines(filename , n = 2)
      comma1 <- charCount(",", _testline[1])
      semicol1 <- charCount(";", _testline[1])
      if (comma1 > 0) {
        _dfloaded <- utils::read.csv(filename,as.is = !factorise,...)
      } else if (semicol1  > 0) {
        _dfloaded <- utils::read.csv2(filename,as.is = !factorise,...)
      } else {
  ...
  # Stata Mac and Linux  use a different default encoding, "latin1". 
      _dfloaded <- haven::read_dta(filename)    # encoding = "latin1"
    } else if (ext == "rec") {
      # foreign packages is required
      ...
      _dfloaded <- foreign::read.epiinfo(filename)
    } else if (ext == "rda" | ext == "rdata" ) {
      # load return name and load content into selected env
      _dfloaded <- load(filename)
      _dfloaded <- get(_dfloaded)
      ...
      _dfloaded <- read_excel(filename)
    } else {
      cat("Extension '", ext, "'not found")
    }
    if (!missing(label)) {
      attr(_dfloaded, "label") <- label
    }
    if (is.data.frame(_dfloaded)) {
      fileatt <- dim(_dfloaded)
      if (lowercase) {
        names(_dfloaded)<-tolower(names(_dfloaded))
      }
